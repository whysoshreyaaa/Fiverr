...
async def search(
    ...
    try:
        ...
        sort_clause = [{"_score": {"order": "desc"}}]  # Primary sort by relevance
        if sortOrder:
            sort_clause.append({
                "JudgmentMetadata.CaseDetails.JudgmentYear.keyword": {  # Numeric field
                    "order": sortOrder  # asc/desc from frontend
                }
            })
